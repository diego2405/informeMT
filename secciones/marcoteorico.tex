...
%La identificación (descriptor) de una persona detectada es almacenada en una galería. Si el tamaño de la galería evoluciona con el tiempo, se dice que el sistema de Re-ID es de \emph{conjunto abierto}, en caso contrario (galería de tamaño fijo), el sistema es de \emph{conjunto cerrado}.

%Cada vez que se detecta una persona en la escena, su descriptor es consultado con la galería en busca de una correspondencia. Para el caso de galería cerrada, el problema radica en asociar correctamente el descriptor consultado con su \emph{match} correspondiente. En el caso de una galería abierta, primero se debe determinar si la persona existe en la galería (\emph{novelty detection}), agregando el descriptor a la galería en caso de que no exista, o asociándolo su match.
